...
import com.tencent.weibo.sdk.android.R;
...
public class Util {
     ...
     */
    public static AppAuthConfig getConfig(Context context) {
        AppAuthConfig config = new AppAuthConfig();
        String tencentAuth = context.getString(R.string.share_tencent_app_key);

        try {
            String[] authConfig = tencentAuth.split("\n");
            config.setApp_key(Long.valueOf(authConfig[0]));
            config.setApp_key_sec(authConfig[1]);
            if (authConfig.length == 3) {
                config.setRedirect_uri(authConfig[2]);
            }
        } catch (Exception e) {
    ...
    public static class AppAuthConfig {
        ...
        void setApp_key_sec(String app_key_sec) {
            this.app_key_sec = app_key_sec;
        }
