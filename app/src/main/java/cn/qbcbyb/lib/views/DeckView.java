...
public class DeckView extends FrameLayout implements /*TaskStack.TaskStackCallbacks,*/
        DeckViewScroller.DeckViewScrollerCallbacks,
    ...
    ViewPool mViewPool;
    List<DeckChildViewTransform> mCurrentTaskTransforms = new ArrayList<DeckChildViewTransform>();
    ...
    SparseArray<DeckChildView> mTmpTaskViewMap = new SparseArray<DeckChildView>();
...
//                    DeckChild tv = (DeckChild) getChildAt(i);
...
//        });
    ...
    void reset() {
        ...
        if (mViewPool != null) {
            Iterator<DeckChildView> iter = mViewPool.poolViewIterator();
            if (iter != null) {
                while (iter.hasNext()) {
...
//        }
     ...
     */
    public DeckChildView getChildViewForTask(int key) {
        int childCount = getChildCount();
        for (int i = 0; i < childCount; i++) {
            DeckChildView tv = (DeckChildView) getChildAt(i);
            if (tv.getAttachedKey() == key) {
                return tv;
            }
        }
        return null;
     ...
     */
    private boolean updateStackTransforms(List<DeckChildViewTransform> taskTransforms,
                                          List data,
                                          float stackScroll,
                                          int[] visibleRangeOut,
        ...
        for (int i = taskCount - 1; i >= 0; i--) {
            DeckChildViewTransform transform =
                    mLayoutAlgorithm.getStackTransform(i,
    ...
    boolean synchronizeStackViewsWithModel() {
        if (mStackViewsDirty) {
            ...
            for (int i = visibleRange[0]; isValidVisibleRange && i >= visibleRange[1]; i--) {
                Object dataValue = data.get(i);
                DeckChildViewTransform transform = mCurrentTaskTransforms.get(i);
                DeckChildView tv = mTmpTaskViewMap.get(i);

                if (tv == null) {
                    // TODO Check
                    tv = mViewPool.pickUpViewFromPool(i, i);
...
//                tv.getViewBounds().setClipBottom(0);
    ...
    void focusTask(int childIndex, boolean scrollToNewPosition, final boolean animateFocusedState) {
        ...
        if (0 <= childIndex && childIndex < data.size()) {
            ...
            Object dataValue = data.get(childIndex);
            DeckChildView tv = getChildViewForTask(childIndex);
            ...
            if (tv != null) {
                ...
                postScrollRunnable = new Runnable() {
                    ...
                    public void run() {
                        ...
                        DeckChildView tv = getChildViewForTask(mFocusedTaskIndex);
                        if (tv != null) {
                            tv.setFocusedTask(animateFocusedState);
                        }
                    }
            ...
            if (scrollToNewPosition) {
                float newScroll = mLayoutAlgorithm.getStackScrollForTask(childIndex) - 0.5f;
    ...
    public void dismissFocusedTask() {
        ...
        Object dataValue = mCallback.getData().get(mFocusedTaskIndex);
        DeckChildView tv = getChildViewForTask(mFocusedTaskIndex);
        tv.dismissTask();
    }
    ...
    public void onInitializeAccessibilityEvent(AccessibilityEvent event) {
        ...
        if (childCount > 0) {
            ...
            event.setFromIndex(backMostTask.getAttachedKey());
            event.setToIndex(frontMostTask.getAttachedKey());
        }
    ...
    public int getCurrentChildIndex() {
        ...
        if (frontMostChild != null) {
            return frontMostChild.getAttachedKey();
        }
...
//    }
    ...
    public boolean isTransformedTouchPointInView(float x, float y, DeckChildView child) {
...
//        mUIDozeTrigger.poke();
    ...
    public DeckChildView createView(Context context) {
        return mCallback.onCreateNewDeckChildView(context, this);
    }
...
//        tv.onDataUnloaded();
    ...
    @Override
    public void prepareViewToLeavePool(DeckChildView dcv, int key, boolean isNewView) {
        ...
        // Load the task data
        mCallback.loadViewData(new WeakReference<DeckChildView>(dcv), key);
...
//        }
        ...
        if (position != -1) {
            int childCount = getChildCount();
            for (int i = 0; i < childCount; i++) {
                int otherKey = ((DeckChildView) getChildAt(i)).getAttachedKey();
                int pos = otherKey;
                if (position < pos) {
                    insertIndex = i;
                    break;
...
//        dcv.setCallbacks(this);
    ...
    @Override
    public boolean hasPreferredData(DeckChildView tv, int preferredData) {
        return (tv.getAttachedKey() != -1 && tv.getAttachedKey() == preferredData);
    }

//    @Override
//    public void onDeckChildViewDismissed(DeckChild dcv) {
//        boolean taskWasFocused = dcv.isFocusedTask();
//
//        int key = dcv.getAttachedKey();
//        int taskIndex = key;
//
...
//            if (nextTaskIndex >= 0) {
//                DeckChild nextTv = getChildViewForTask(nextTaskIndex);
//                if (nextTv != null) {
...
//    }
    ...
    public void onStackTaskRemoved(DeckChildView removedView) {
        ...
        if (removedView != null) {
            int key = removedView.getAttachedKey();
            int removedPosition = key;
        ...
        int newFrontMostTask = mCallback.getData().get(mCallback.getData().size() - 1);
        // Update the new front most task
        if (newFrontMostTask != null) {
            DeckChild frontTv = getChildViewForTask(newFrontMostTask);
            if (frontTv != null) {
    ...
    public void notifyDataSetChanged() {
        ...
        boolean pullStackForward = mCallback.getData().size() > 0;
        if (pullStackForward) {
            anchorTask = mCallback.getData().size() - 1;
            prevAnchorTaskScroll = mLayoutAlgorithm.getStackScrollForTask(anchorTask);
        }
        ...
        int newFrontMostTask = mCallback.getData().size() > 0 ?
                mCallback.getData().size() - 1
                : -1;
        Object dataValue = mCallback.getData().get(newFrontMostTask);
        // Update the new front most task
        if (dataValue != null) {
...
//        mUIDozeTrigger.poke();
    ...
    public void notifyDataSetChangedOld() {
        ...
        // Get the stack scroll of the task to anchor to (since we are removing something, the front
        // most task will be our anchor task)
        int anchorTaskIndex = -1;
        float prevAnchorTaskScroll = 0;
        boolean pullStackForward = data.size() > 0;
        if (pullStackForward) {
//            anchorTaskIndex = data.get(data.size() - 1);
            prevAnchorTaskScroll = mLayoutAlgorithm.getStackScrollForTask(anchorTaskIndex);
        }
        ...
        // Offset the stack by as much as the anchor task would otherwise move back
        if (pullStackForward) {
            float anchorTaskScroll = mLayoutAlgorithm.getStackScrollForTask(anchorTaskIndex);
            mStackScroller.setStackScroll(mStackScroller.getStackScroll() + (anchorTaskScroll
                    - prevAnchorTaskScroll));
            mStackScroller.boundScroll();
        }
        ...
        int newFrontMostIndex = data.size() - 1;
        Object newFrontMostData = data.get(newFrontMostIndex);
        // Update the new front most task
        if (newFrontMostData != null) {
            DeckChildView frontTv = getChildViewForTask(newFrontMostIndex);
            if (frontTv != null) {
                frontTv.onTaskBound(newFrontMostIndex);
            }
        }
    ...
    public interface Callback {
        ...
        void loadViewData(WeakReference<DeckChildView> dcv, int item);

        void unloadViewData(int item);

        void onViewDismissed(int item);

        void onItemClick(int item);

        void onNoViewsToDeck();

        DeckChildView onCreateNewDeckChildView(Context context, DeckView deckView);
    }
}
