...
public class DeckViewLayoutAlgorithm {
    ...
    SparseArray<Float> mTaskProgressMap = new SparseArray<Float>();
     ...
     */
    void computeMinMaxScroll(List data, boolean launchedWithAltTab,
     ...
     */
    public VisibilityReport computeStackVisibilityReport(List data) {
        if (data.size() <= 1) {
        ...
        float progress = mTaskProgressMap.get(data.size() - 1) - mInitialScrollP;
        int prevScreenY = curveProgressToScreenY(progress);
        for (int i = data.size() - 2; i >= 0; i--) {
            //Task task = tasks.get(i);
            //progress = mTaskProgressMap.get(task.key) - mInitialScrollP;
            progress = mTaskProgressMap.get(i) - mInitialScrollP;
            if (progress < 0) {
                break;
            }
     ...
     */
    public DeckChildViewTransform getStackTransform(int key, float stackScroll,
                                                    DeckChildViewTransform transformOut,
                                                    DeckChildViewTransform prevTransform) {
        // Return early if we have an invalid index
        if (mTaskProgressMap.indexOfKey(key) < 0) {
            transformOut.reset();
            return transformOut;
        }
     ...
     */
    public float getStackScrollForTask(int key) {
        if (mTaskProgressMap.indexOfKey(key) < 0) return 0f;
