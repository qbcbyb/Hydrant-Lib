 ...
 * Time: 12:31 PM
 */
public class DynamicGridView extends GridViewWithHeaderAndFooter {
    ...
    private Map<View, ObjectAnimator> mResizeAnimators = new HashMap<View, ObjectAnimator>();
    ...
    public float getScaleInEdit() {
        return scaleInEdit;
    }

    public void setScaleInEdit(float scaleInEdit) {
        this.scaleInEdit = scaleInEdit;
    }
    ...
    private void startWobbleAnimation() {
        int childCount = Math.min(getChildCount(), getAdapterInterface().getItemCount() + getHeaderViewCount() * getColumnCount() - getFirstVisiblePosition());
        int firstVisible = Math.max(0, getHeaderViewCount() * getColumnCount() - getFirstVisiblePosition());
        for (int i = firstVisible; i < childCount; i++) {
            View v = getChildAt(i);
    ...
    private void stopWobble(boolean resetRotation) {
        for (ObjectAnimator wobbleAnimator : mWobbleAnimators) {
            wobbleAnimator.cancel();
            View v = (View) wobbleAnimator.getTarget();
            if (v != null) {
                if (resetRotation) {
        ...
        if (resetRotation) {
            synchronized (mResizeAnimators) {
                for (Map.Entry<View, ObjectAnimator> entry : mResizeAnimators.entrySet()) {
                    ObjectAnimator resizeAnimator = entry.getValue();
                    resizeAnimator.cancel();
                    View v = entry.getKey();
                    resetScaleViewInWobble(v);
                }
                mResizeAnimators.clear();
            }
        }
    }
    ...
    OnScrollListener outScrollListener;
    ...
    private void animateWobble(View v) {
        ...
        animator.setFloatValues(-1.5f, 1.5f);
        animator.start();
    ...
    private void scaleViewInWobble(final View child) {
        if (mResizeAnimators.containsKey(child)) {
            return;
        }
        ObjectAnimator objectAnimator = startScaleAnimator(child, scaleInEdit);
        synchronized (mResizeAnimators) {
            mResizeAnimators.put(child, objectAnimator);
        }
    }

    private void resetScaleViewInWobble(View child) {
        startScaleAnimator(child, 1f);
    }

    private ObjectAnimator startScaleAnimator(final View child, float e) {
        ObjectAnimator objectAnimator = ObjectAnimator.ofFloat(child, new FloatProperty<View>(null) {
            @Override
    ...
    private void reorderElements(int originalPosition, int targetPosition) {
        int originalPosition1 = originalPosition - getHeaderViewCount() * getColumnCount();
        int newPosition = targetPosition - getHeaderViewCount() * getColumnCount();
        if (mDragListener != null)
            mDragListener.onDragPositionsChanged(originalPosition1, newPosition);
        getAdapterInterface().reorderItems(originalPosition1, newPosition);
    }

    @Override
    protected int getNumColumnsCompatible() {
        if (Build.VERSION.SDK_INT >= 11) {
            return super.getNumColumnsCompatible();
        } else {
    ...
    protected int getColumnWidthCompatible() {
        if (Build.VERSION.SDK_INT >= 16) {
            return super.getColumnWidthCompatible();
        } else {
            return getWidth() / getNumColumnsCompatible();
        }
    }
    ...
    public DynamicGridAdapterInterface getAdapterInterface() {
        ...
        if (adapter instanceof DynamicGridAdapterInterface) {
            ...
            return ((DynamicGridAdapterInterface) ((WrapperListAdapter) adapter).getWrappedAdapter());
        }
        return null;
    }
    ...
    private void updateNeighborViewsForId(long itemId) {
        ...
        for (int pos = Math.max(getFirstVisiblePosition(), getHeaderViewCount() * getColumnCount()); pos <= Math.min(getLastVisiblePosition(), getAdapterInterface().getItemCount() + getHeaderViewCount() * getColumnCount() - 1); pos++) {
            if (draggedPos != pos) {
        ...
        switch (event.getAction() & MotionEvent.ACTION_MASK) {
            case MotionEvent.ACTION_DOWN:
                ...
                if (mIsEditMode && isEnabled()) {
                    requestDisallowInterceptTouchEvent(true);
                    layoutChildren();
                    int position = pointToPositionCompact(mDownX, mDownY);//pointToPosition(mDownX, mDownY);
                    startDragAtPosition(position);
                    return true;
                } else if (!isEnabled()) {
                    return false;
    ...
    public int pointToPositionCompact(int x, int y) {
        ...
        for (int i = count - 1; i >= 0; i--) {
            ...
            if (child.getVisibility() == View.VISIBLE) {
                child.getDrawingRect(frame);
                frame.offset(child.getLeft(), child.getTop());
    ...
    private void startDragAtPosition(int position) {
        mTotalOffsetY = 0;
        mTotalOffsetX = 0;
        if (position < getHeaderViewCount() * getColumnCount() || position >= getAdapterInterface().getItemCount() + getHeaderViewCount() * getColumnCount()) {
            return;
        }
        ...
        if (selectedView != null) {
            ...
            if (mDragListener != null) {
                int originalPosition = position - getHeaderViewCount() * getColumnCount();
                mDragListener.onDragStarted(originalPosition);
            }
        }
    }
    ...
    private void reset(View mobileView) {
        ...
        mHoverCell = null;
        if (!mIsEditMode && mWobbleInEditMode)
            stopWobble(true);
        if (mIsEditMode && mWobbleInEditMode)
            restartWobble();
        invalidate();
    ...
    private OnScrollListener mScrollListener = new OnScrollListener() {
        ...
        private void updateWobbleState(int visibleItemCount) {
            ...
            visibleItemCount = Math.min(visibleItemCount, getAdapterInterface().getItemCount() + getHeaderViewCount() * getColumnCount() - getFirstVisiblePosition());
            int firstVisible = Math.max(0, getHeaderViewCount() * getColumnCount() - getFirstVisiblePosition());
            for (int i = firstVisible; i < visibleItemCount; i++) {
                ...
                if (child != null) {
                    if (mMobileItemId != INVALID_ID && Boolean.TRUE != child.getTag(R.id.dgv_wobble_tag)) {
                        ...
                        scaleViewInWobble(child);
                        child.setTag(R.id.dgv_wobble_tag, true);
                    } else if (mMobileItemId == INVALID_ID && ViewHelper.getRotation(child) != 0) {
                        ViewHelper.setRotation(child, 0);
                        child.setTag(R.id.dgv_wobble_tag, false);
//                    } else if (!isEditMode()) {
//                        resetScaleViewInWobble(child, true);
                    }
                }
